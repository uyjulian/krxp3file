	...
	tjs_uint64 & offset, bool raise)
{
	...
	if(mark[0] == 0x4d/*'M'*/ && mark[1] == 0x5a/*'Z'*/)
	{
		...
		{
			if(raise)
				TVPThrowExceptionMessage(TJS_W("Cannot unbind XP3 exe %1"), name);
			else
				return false;
		}
	}
	...
	{
		if(raise)
			TVPThrowExceptionMessage(TJS_W("%1 is not XP3 archive or unsupported"), name);
		return false;
	}
...
tTVPXP3Archive::tTVPXP3Archive(const ttstr & name) : tTVPArchive(name)
{
	...
	TVPAddLog( TVPFormatMessage(TJS_W("(info) Trying to read XP3 virtual file system information from : %1"), name) );
				...
				try
				{
					...
					if(result != Z_OK ||
						destlen != (unsigned long)index_size)
							TVPThrowExceptionMessage(TJS_W("Unpompression failed"));
				}
				catch(...)
				{
					delete [] compressed;
			...
			for(;;)
			{
				...
				for(tjs_int i = 0; i<segment_count; i++)
				{
					...
					if((flags & TVP_XP3_SEGM_ENCODE_METHOD_MASK) ==
						...
						TVPThrowExceptionMessage(TJS_W("Read error")); // unknown encode method
	...
	catch(...)
	{
		if(indexdata) delete [] indexdata;
		delete st;
 		TVPAddLog( (const tjs_char*)TJS_W("(info) Failed.") );
		throw;
	}
	if(indexdata) delete [] indexdata;
	delete st;

	TVPAddLog( TVPFormatMessage( TJS_W("(info) Done. (contains %1 file(s), %2 segment(s))"), ttstr(Count), ttstr(segmentcount) ) );
}
//---------------------------------------------------------------------------
...
{
	if(idx >= ItemVector.size()) TVPThrowExceptionMessage(TJS_W("Read error"));
...
public:
	...
	{
		tjs_uint32 v = TJS::tTJSHashFunc<ttstr>::Make(val.Name);
...
typedef
TJS::tTJSHashTable<tTVPSegmentCacheSearchData, tTVPSegmentDataHolder, tTVPSegmentCacheSearchHashFunc>
	tTVPSegmentCache;
